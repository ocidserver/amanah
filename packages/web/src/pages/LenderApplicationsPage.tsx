  ...
  return (
    <div className="px-4 pt-4 pb-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">Pengajuan Pinjaman</h1>
      </div>

      {pendingApplications.length > 0 && (
        <div className="mb-6">
          <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Menunggu Persetujuan</h2>
      ...
      {approvedLoans.length > 0 && (
        <div className="mb-6">
          <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Disetujui — Menunggu Aktivasi</h2>
          <div className="space-y-3">
            {approvedLoans.map((loan) => (
              <div key={loan.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-blue-100 dark:border-blue-800 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 font-medium">Disetujui</span>
                    <p className="text-lg font-bold text-gray-900 dark:text-gray-100 mt-1">{formatCurrency(loan.amount)}</p>
                    <p className="text-xs text-gray-400 dark:text-slate-500">{loan.durationMonths} bulan — {LOAN_PURPOSE[loan.purpose as keyof typeof LOAN_PURPOSE]}</p>
                  </div>
                  <button
                    onClick={() => navigate(`/pinjaman/${loan.id}`)}
                    className="bg-blue-600 dark:bg-blue-700 text-white text-sm px-4 py-2 rounded-xl font-semibold"
                  >
                    Kelola
                  </button>
                </div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2].map((i) => (
            <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 animate-pulse h-32" />
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-8 text-center">
          ...
          <p className="text-gray-400 dark:text-slate-500 text-sm">Belum ada pengajuan pinjaman</p>
          <p className="text-gray-400 dark:text-slate-500 text-xs mt-1">Pengajuan dari peminjam akan muncul di sini</p>
        </div>
      ) : null}
    </div>
  )
...
function ApplicationCard({ application }: { application: ApplicationItem }) {
  ...
  return (
    <button
      ...
      className="block w-full text-left bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 active:scale-[0.99] transition-transform"
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-medium">Menunggu</span>
          ...
          <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{formatCurrency(application.amount)}</p>
          ...
          {borrower && (
            <div className="mt-2 flex items-center gap-2">
              <span className="text-xs text-gray-500 dark:text-slate-400">{borrower.displayName || borrower.email}</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400">{tierLabel}</span>
            </div>
          )}
