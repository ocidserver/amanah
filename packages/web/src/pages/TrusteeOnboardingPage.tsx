      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {step === 1 && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-5">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">Data Pribadi</h2>
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]" placeholder="Nama sesuai KTP" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Alamat <span className="text-red-500 dark:text-red-400">*</span></label>
                <textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={2} className="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-[var(--color-primary)] resize-none" placeholder="Alamat lengkap" />
              </div>
              <div>
                ...
                <input type="text" value={occupation} onChange={(e) => setOccupation(e.target.value)} className="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]" placeholder="Jenis pekerjaan" />
              </div>

              {/* KTP Upload */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Foto KTP <span className="text-red-500 dark:text-red-400">*</span></label>
                <input type="file" ref={fileInputRef} accept="image/jpeg,image/png,image/webp" onChange={handleKtpSelect} className="hidden" />
                {ktpPreview ? (
                  <div className="relative">
                    <img src={ktpPreview} alt="KTP" className="w-full h-40 object-cover rounded-xl border border-gray-200 dark:border-slate-700" />
                    <button
                  ...
                  <button
                    ...
                    className="w-full border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-xl py-8 flex flex-col items-center gap-2 hover:border-[var(--color-primary)] transition-colors"
                  >
                    <IconUpload className="w-8 h-8 text-gray-400 dark:text-slate-500" />
                    <span className="text-sm text-gray-500 dark:text-slate-400">Upload foto KTP</span>
                    <span className="text-xs text-gray-400 dark:text-slate-500">JPG, PNG, WebP (max 5MB)</span>
                  </button>
                )}
              </div>
        ...
        {step === 2 && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-5">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">Profil Wali Amanah</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Tipe Wali <span className="text-red-500 dark:text-red-400">*</span></label>
              ...
              {type === "institution" && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Nama Institusi <span className="text-red-500 dark:text-red-400">*</span></label>
                  <input type="text" value={institution} onChange={(e) => setInstitution(e.target.value)} className="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]" placeholder="Contoh: BMT Amanah Sejahtera" />
                </div>
              )}

              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Email <span className="text-gray-400 dark:text-slate-500 font-normal">(opsional)</span></label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]" placeholder="email@contoh.com" />
              </div>

              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
                <p className="text-sm font-medium text-blue-900 dark:text-blue-400">Tanggung Jawab Wali Amanah</p>
