...
import { IconChevronLeft, IconClock, IconCheck, IconUpload, IconDownload } from "../components/Icons"
...
export default function BorrowerLoanDetailPage() {
  ...
  const uploadProofMutation = useMutation({
    mutationFn: async ({ installmentId, file }: { installmentId: string; file: File }) => {
      ...
      return api.upload<{ proof: { id: string; imageUrl: string; status: string } }>(`/payment-proofs/installments/${installmentId}/upload`, formData)
    },
  ...
  if (!data?.loan) {
    return (
      <div className="px-4 pt-4">
        <p className="text-gray-400 dark:text-slate-500 text-center">Pinjaman tidak ditemukan</p>
  ...
  return (
    <div className="px-4 pt-2 pb-6">
      <Link to="/borrower" className="inline-flex items-center gap-1 text-sm text-gray-500 dark:text-slate-400 mb-4">
      ...
      {loan.contractUrl && (
        <a
          ...
          className="flex items-center justify-center gap-2 w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-slate-300 mb-4 hover:bg-gray-50 dark:hover:bg-slate-700 active:scale-[0.98] transition-transform"
        >
      ...
      <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Jadwal Cicilan</h3>
      {installments.length === 0 ? (
        <p className="text-gray-400 dark:text-slate-500 text-sm">Belum ada cicilan</p>
      ) : (
        <div className="space-y-2">
          {installments.map((inst) => (
            <div key={inst.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 px-4 py-3 flex items-center gap-3">
              {inst.status === "paid" ? (
              ...
              <div className="text-right shrink-0 min-w-[100px]">
                ...
                {inst.status === "paid" ? (
                  ...
                  <div className="mt-1">
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-slate-400 mt-1">Mengupload...</p>
                  </div>
        ...
        <div className="mt-6">
          ...
          {completionMessage ? (
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-xl p-4">
              <p className="text-sm text-green-800 dark:text-green-400 italic">"{completionMessage.message}"</p>
            ...
            <form onSubmit={handleDoaLunas} className="space-y-3">
              <textarea
                ...
                maxLength={500}
              />
              {doaError && <p className="text-red-600 dark:text-red-400 text-xs">{doaError}</p>}
              <button
                type="submit"
            ...
            <p className="text-gray-400 dark:text-slate-500 text-sm">Kirim doa lunas setelah semua cicilan terbayar.</p>
          )}
        </div>
      )}
...
function RateLenderForm({ loanId, lenderId, onSubmitted }: { loanId: string; lenderId: string; onSubmitted: () => void }) {
  ...
  if (submitted) {
    return (
      <div className="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-xl p-4 text-center">
        <p className="text-green-800 dark:text-green-400 font-semibold">Terima kasih!</p>
        <p className="text-green-600 dark:text-green-400 text-sm mt-1">Rating Anda telah terkirim.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Beri Rating Pemberi Pinjaman</h3>
