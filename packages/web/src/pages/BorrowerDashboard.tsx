  ...
  return (
    <div className="px-4 pt-4 pb-4">
      <div className="mb-5">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{user?.displayName || "Peminjam"}</h2>
      ...
      {showOnboardingBanner && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 mb-6">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Lengkapi Profil Anda</h3>
              ...
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                  ...
                  {biStatus === "approved" ? (
                    ...
                    <IconClock className="w-4 h-4 text-yellow-600 dark:text-yellow-400" />
                  ) : (
                    <IconShield className="w-4 h-4 text-gray-400 dark:text-slate-500" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {biStatus === "pending" && (
                    <p className="text-xs text-yellow-600 dark:text-yellow-400">Sedang dalam proses...</p>
                  )}
                </div>
      ...
      {isLoading ? (
        ...
        <>
            ...
            <div className="mb-6">
              {pendingLoans.length > 0 && (
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 mb-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-amber-800 dark:text-amber-400">{pendingLoans.length} Pengajuan Menunggu</p>
                      <p className="text-sm text-amber-600 dark:text-amber-400">Sedang ditinjau oleh lender</p>
                    </div>
                    <Link to="/borrower/pengajuan" className="text-sm text-amber-700 dark:text-amber-400 font-medium">
                      Lihat →
                    </Link>
                  </div>
                </div>
              ...
              {approvedLoans.length > 0 && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-4 mb-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-blue-800 dark:text-blue-400">{approvedLoans.length} Pinjaman Disetujui</p>
                      <p className="text-sm text-blue-600 dark:text-blue-400">Menunggu pencairan</p>
                    </div>
                    <Link to="/borrower/pengajuan" className="text-sm text-blue-700 dark:text-blue-400 font-medium">
                      Lihat →
                    </Link>
                  </div>
                </div>
          ...
          {(pendingLoans.length > 0 || approvedLoans.length > 0) && (
            <div className="mb-6">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Status Pengajuan</h3>
              <div className="space-y-2">
                {pendingLoans.map((loan) => (
                  <Link
                    key={loan.id}
                    to={`/borrower/pinjaman/${loan.id}`}
                    className="block bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-medium">Menunggu</span>
                        <p className="text-sm font-bold text-gray-900 dark:text-gray-100 mt-1">{formatCurrency(loan.amount)}</p>
                      </div>
                      <IconChevronRight className="w-5 h-5 text-amber-400 dark:text-amber-500" />
                    </div>
                  </Link>
                ))}
                {approvedLoans.map((loan) => (
                  <Link
                    key={loan.id}
                    to={`/borrower/pinjaman/${loan.id}`}
                    className="block bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 font-medium">Disetujui</span>
                        <p className="text-sm font-bold text-gray-900 dark:text-gray-100 mt-1">{formatCurrency(loan.amount)}</p>
                      </div>
                      <IconChevronRight className="w-5 h-5 text-green-400 dark:text-green-500" />
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Pinjaman Saya</h3>
          </div>
          ...
          {loans.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-8 text-center">
              ...
              <p className="text-gray-400 dark:text-slate-500 text-sm">Belum ada pinjaman</p>
              <p className="text-gray-400 dark:text-slate-500 text-xs mt-1">Ajukan pinjaman pertama Anda sekarang</p>
            </div>
          ) : (
            <div className="space-y-3">
              {loans.slice(0, 5).map((loan) => (
                <Link
                  key={loan.id}
                  to={`/borrower/pinjaman/${loan.id}`}
                  className="block bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 active:scale-[0.99] transition-transform"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
